...
from ..parameterExtraction.bminchScripts import ekvfitFile
...
def prepareDCSweepSimulation(ascFilePath, parent_dir, sweeptype="linear", start=0, stop=3.3, nrPoints=1000):
    ...
    if sweeptype.lower() == "linear":
        ...
        hybridSpacedList = hybrid_spaced_list(start, stop, nrPoints, 0.5, 0.65, 0.1, 1)
...
# ascEditedFile = prepareDCSweepSimulation(ascFilePath, simOutputPath, sweeptype="hybrid", start=-2, stop=3.3, nrPoints=300)

# runSimulation(ascEditedFile, simOutputPath, f"ekvParameterExtractionEdited_{mosType}.net")

raw = RawRead(os.path.join(simOutputPath, "ekvParameterExtractionEdited_nmos_100pts.raw"))
...
Is, VT, kappa = ekvfit(xdata, ydata, 5e-4, plotting="on")
