...
def gradientDescent(x, y, theta, W, L, alpha, num_iters):
    """
    ...
    update theta params using the following formula:
        theta(x,j) = theta(j) - alpha(j) * derivative of the cost function with respect to theta(j)
        theta(x,j) = theta(j) - alpha(j) * dJ(theta) / dtheta(j)
    ...
    # Create a copy of theta to avoid changing the original array
    theta = theta.copy()

    cost_history = []  # Use a list to save cost in every iteration

    def ekv(vg, is_val, vth0_val, kappa_val, W, L):
        return is_val*(W/L)*(np.log(1 + np.exp(kappa_val * (vg - vth0_val) / (2 * 0.0258))))**2

    def lossFunction(x, theta, y):
        """
    ...
    for j in range(num_iters):
        # Compute the cost function derivative and update each theta parameter for every input data point (VG value)
        ...
        # theta[0] is the current Is value
        # theta[1] is the current VTh0 value
        # theta[2] is the current kappa
        # for j in range(len(x)):
            
        
        # VG = x[i]

         # Plot the current approximation of the mos drain current using the current theta values
        if line:
            line.remove()  # Remove the previous line
        line, = ax.plot(x, [ekv(i, Is, Vth0, k, W, L) for i in x], label=f"Iteration {j}")
        ax.legend()
        ...
        Is = Is - alpha_Is * (2 / m) * np.sum([lossFunction(x[i], [Is, Vth0, k], y[i])*dekv_dIs(x[i], W, L, k, Vth0) for i in range(len(x))])
        k = k - alpha_k * (2 / m) * np.sum([lossFunction(x[i], [Is, Vth0, k], y[i])*dekv_dk(x[i], W, L, k, Vth0) for i in range(len(x))])
        Vth0 = Vth0 - alpha_Vth0 * (2 / m) * np.sum([lossFunction(x[i], [Is, Vth0, k], y[i])*dekv_dVth0(x[i], W, L, k, Vth0) for i in range(len(x))])
        ...
        cost = (1/m)*np.sum([lossFunction(x[i], [Is, Vth0, k], y[i])**2 for i in range(len(x))])
